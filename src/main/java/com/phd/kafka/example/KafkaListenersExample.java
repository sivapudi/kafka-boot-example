...
import com.phd.kafka.util.User;

public class KafkaListenersExample {

	private final Logger logger = LoggerFactory.getLogger(KafkaListenersExample.class);

	@KafkaListener(topics = "phd-1")
	void listener(String message) {
		logger.info("Listener [{}]", message);
	}

	@KafkaListener(topics = { "phd-1", "phd-2" }, groupId = "phd-group-2")
	void commonListenerForMultipleTopics(String message) {
		logger.info("MultipleTopicListener - [{}]", message);
	}

	@KafkaListener(topicPartitions = @TopicPartition(topic = "phd-3", partitionOffsets = {
